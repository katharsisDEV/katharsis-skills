#!/usr/bin/env python3
"""Détecte le contenu rogné ou sous le pied de page dans un deck HTML, et contrôle les liens d'un PDF.
...
(function () {
  ...
  function measure() {
    ...
    document.querySelectorAll('.slide').forEach(function (slide, si) {
      ...
      // Un bloc peut aussi descendre sous le filet du pied de page : rien n'est
      // coupé, mais il chevauche la signature de la page.
      var foot = slide.querySelector('.foot');
      if (foot) {
        var limit = foot.getBoundingClientRect().top - 3;
        slide.querySelectorAll('.card, .stats, .callout, .banner, .dgwrap, table, svg.dg, .legend')
          .forEach(function (el) {
            if (el.parentElement.closest('.card')) return;
            var o = el.getBoundingClientRect().bottom - limit;
            if (o > 1) {
              var l = el.querySelector('.lbl, .ct');
              cut.push({ page: page,
                         label: (l ? l.textContent : String(el.className.baseVal !== undefined ? el.className.baseVal : el.className) || el.tagName).trim().slice(0, 64),
                         lost: Math.round(o), text: '(bloc sous le filet du pied de page)' });
            }
          });
      }
